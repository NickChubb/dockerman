...
router.delete('/:containerId/remove', (req, res) => {
    ...
    container.remove((err, data) => {
      if (!err) {
        ...
        log(`📦🚫 Failed to remove container with id: ${id}.  ERROR: ${err.message}`);
        res.sendStatus(500);
      }
    });
});
...
const startContainer = (container, id) => {
  ...
  container.start((err, data) => {
    if (!err) {
      ...
      log(`📦🚫 Failed to started container with id: ${id}.  ERROR: ${err.message}`);
      return false;
    }
  });
...
const stopContainer = (container, id) => {
  ...
  container.stop((err, data) => {
    if (!err) {
      ...
      log(`📦🚫 Failed to stop container with id: ${id}.  ERROR: ${err.message}`);
      return false;
    }
  });
...
const restartContainer = (container, id) => {
  ...
  container.restart((err, data) => {
    if (!err) {
      ...
      log(`🔄🚫 Failed to restart container with id: ${id}.  ERROR: ${err.message}`);
      return false;
    }
  });
