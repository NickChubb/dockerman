 ...
 * 
 * Considered MongoDB but SQLite3 doesn't require any extra services running AFAIK
 */
class Database  {

    constructor() {

        // Get a sequelize database, or create if it doesn't exist
        this.sequelize = new Sequelize('database', 'user', 'password', {
            host: 'localhost',
            dialect: 'sqlite',
            logging: false,
            // SQLite only
            storage: 'data/database.sqlite', // Storage location
        });

        /**
         *  Defines the database for the authentication token. 
         * 
         *  Authentication token is UUIDv4 and generated upon a successful login.
         *  Used to compare with token attempting to access Dockerman.
         */
     ...
     */
    async getServices() {
        const services = await this.services.findAll({ order: [['slug', 'ASC']] });
        return services;
    }
    ...
    // @TODO: get log entries corresponding to page and size
    ...
    async refreshTokens() {
        const response = await this.token.destroy({
            where: {
                createdBy: {
                    $lte: DateTime.now().minus({days: 7}).toISO()
                }
            }
        });
